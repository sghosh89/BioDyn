library(brms)
library(tidyverse)
reslocn<-"../../Results/gather_res/res_taxa15/LT_and_UT_sep_traditional_stability/"
if(!dir.exists(reslocn)){
  dir.create(reslocn)
}


call_toymodel_fixed_realm_LT_and_UT_sep_trad_stab<-function(i){
  
  mydat_scaled<-readRDS(paste("../../Results/gather_res/res_taxa15/run_",i,"/reduced_data_taxa15.RDS",sep=""))
  mydat_scaled<-mydat_scaled%>%dplyr::select(REALM, TAXA, UID, iCV,R,VR_LdM,A,L,absU)
  ...
  resloc<-paste("../../Results/gather_res/res_taxa15/LT_and_UT_sep_traditional_stability/run_",i,sep="")
  ...
  bf_stability<-bf(stability ~ (R+L+U+VR_LdM)*REALM + (1|TAXA/UID))
...
for(i in 1:100){
  call_toymodel_fixed_realm_LT_and_UT_sep_trad_stab(i=i)
}
#==================
...
write.csv(taxaeffect,paste("../../Results/gather_res/res_taxa15/LT_and_UT_sep_traditional_stability/randomtaxaeffect_summary_fixed_realm.csv",sep=""),row.names = F)
...
write.csv(fixedeffect,"../../Results/gather_res/res_taxa15/LT_and_UT_sep_traditional_stability/fixedeffectestimate_summary_fixed_realm.csv",row.names = F)

#----------------Now, make table for Summary estimates from 100 runs--------------------------
fixedeffect<-read.csv("../../Results/gather_res/res_taxa15/LT_and_UT_sep_traditional_stability/fixedeffectestimate_summary_fixed_realm.csv")
...
sink("../../Results/gather_res/res_taxa15/LT_and_UT_sep_traditional_stability/100runs_fixedeffects_estimates_table.txt")
print(si3,digits=3)
sink()
