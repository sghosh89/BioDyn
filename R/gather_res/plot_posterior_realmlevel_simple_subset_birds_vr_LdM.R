...
gathered_post <-
  ...
  rename(b_VR_LdM_Fresh=b_VR_LdM,
         `b_VR_LdM_diff_T-F`=`b_VR_LdM:REALMTerrestrial`)%>%
  mutate("b_VR_LdM_Terres" = 
           b_VR_LdM_Fresh + `b_VR_LdM_diff_T-F`)%>%
  gather() %>% 
  mutate(key = factor(key, levels = c("b_VR_LdM_diff_T-F",
                                      "b_VR_LdM_Fresh" , 
                                      "b_VR_LdM_Terres"
  )))
...
#--------------------------------------------------------------------------------
pdf("../../Results/gather_res/simple_subset_birds_21_vr_LdM/plot_posterior.pdf",height=16,width=15)
...
pdf("../../Results/gather_res/simple_subset_birds_21_vr_LdM/plot_posterior_pointinterval.pdf",height=5,width=8)
op<-par(mar=c(2,2,2,2))
...
pdf("../../Results/gather_res/simple_subset_birds_21_vr_LdM/plot_taxaeffect.pdf",height=3,width=7)
op<-par(mar=c(2,2,2,2))
g_tx
par(op)
dev.off()
...
pdf("../../Results/gather_res/simple_subset_birds_21_vr_LdM/conditional_effects.pdf",height=12,width=16)
