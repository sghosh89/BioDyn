...
#========================================================================================
sink("./console_practice_hierarchicalmodel_skwnormalfamily.txt", append=TRUE, split=TRUE)
#====================================================================================================
...
xx<-mydat_scaled%>%group_by(REALM,TAXA,STUDY_ID)%>%count()%>%ungroup()
sum(xx$n)==nrow(mydat)

ggplot(data=mydat_scaled,aes(y=stability_skw,x=R,col=STUDY_ID,shape=REALM))+
...
null_model<-add_criterion(null_model,
                          ...
                          moment_match = T,
                          reloo = T)
saveRDS(null_model,"../../Results/gather_res/pratice_hierarchical_nullmodel_skwnormal.RDS")


cat(paste("------- brms full model starting at time: ", Sys.time()," -------------- \n "))

bf_stability<-bf(stability_skw ~ (R+VR+A+uniA+SR)*REALM+ 
                   (1+(R+VR+A+uniA+SR)*REALM|TAXA/STUDY_ID))
...
full_model<-add_criterion(full_model,
                          ...
                          moment_match = T,
                          reloo = T)
saveRDS(full_model,"../../Results/gather_res/pratice_hierarchical_fullmodel_skwnormal.RDS")
...
#pp_check(null_model,nsamples = 10)
...
brms::pp_check(null_model)
brms::pp_check(full_model)
conditional_effects(full_model)
