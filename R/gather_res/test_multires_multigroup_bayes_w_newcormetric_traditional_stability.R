...
reslocn<-paste("../../Results/gather_res/res_taxa15/multires_multigroup_w_newcormetric_traditional_stability")
...
call_multires_multigroup<-function(i){
  ...
  resloc<-paste("../../Results/gather_res/res_taxa15/multires_multigroup_w_newcormetric_traditional_stability/run_",i,sep="")
  ...
  bf_stability<-bf(stability ~ (R+A+newcormetric)*REALM + (1|TAXA/UID))
...
for(i in c(1:100)){
  ...
  post<-post%>%dplyr::select(b_stability_Intercept,
                             b_stability_R,b_stability_A,b_stability_newcormetric,
                             b_stability_REALMTerrestrial,
                             `b_stability_R:REALMTerrestrial`, `b_stability_A:REALMTerrestrial`, `b_stability_newcormetric:REALMTerrestrial`,
  ...
  df1<-post %>%dplyr::select(b_stability_newcormetric)%>% 
    tidybayes::median_qi(.width = c(.95,0.75))
  df$Median[4]<-df1$b_stability_newcormetric[1]
  ...
  df1<-post %>%mutate(b_stability_newcormetric_terres=b_stability_newcormetric+`b_stability_newcormetric:REALMTerrestrial`)%>% select(b_stability_newcormetric_terres)%>% 
    tidybayes::median_qi(.width = c(.95,0.75))
  df$Median[12]<-df1$b_stability_newcormetric_terres[1]
...
write.csv(fixedeffect,paste("../../Results/gather_res/res_taxa15/multires_multigroup_w_newcormetric_traditional_stability/fixedeffectestimate_summary_fixed_realm.csv",sep=""),row.names = F)

#----------------Now, make table for Summary estimates from 100 runs--------------------------
fixedeffect<-read.csv(paste("../../Results/gather_res/res_taxa15/multires_multigroup_w_newcormetric_traditional_stability/fixedeffectestimate_summary_fixed_realm.csv",sep=""))
...
sink(paste("../../Results/gather_res/res_taxa15/multires_multigroup_w_newcormetric_traditional_stability/100runs_fixedeffects_estimates_table.txt",sep=""))
