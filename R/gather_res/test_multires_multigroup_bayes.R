...
reslocn<-paste("../../Results/gather_res/res_taxa15/multires_multigroup")
...
call_multires_multigroup<-function(i){
  ...
  resloc<-paste("../../Results/gather_res/res_taxa15/multires_multigroup/run_",i,sep="")
  ...
  bf_stability<-bf(stability_skw ~ (R+A+LM)*REALM + (1|TAXA/UID))
...
for(i in c(1:100)){
  ...
  post<-post%>%dplyr::select(b_stabilityskw_Intercept,
                      b_stabilityskw_R,b_stabilityskw_A,b_stabilityskw_LM,
                      b_stabilityskw_REALMTerrestrial,
                      `b_stabilityskw_R:REALMTerrestrial`, `b_stabilityskw_A:REALMTerrestrial`, `b_stabilityskw_LM:REALMTerrestrial`,
  ...
  df1<-post %>%dplyr::select(b_stabilityskw_LM)%>% 
    tidybayes::median_qi(.width = c(.95,0.75))
  df$Median[4]<-df1$b_stabilityskw_LM[1]
  ...
  df1<-post %>%mutate(b_stabilityskw_LM_terres=b_stabilityskw_LM+`b_stabilityskw_LM:REALMTerrestrial`)%>% select(b_stabilityskw_LM_terres)%>% 
    tidybayes::median_qi(.width = c(.95,0.75))
  df$Median[12]<-df1$b_stabilityskw_LM_terres[1]
...
write.csv(fixedeffect,paste("../../Results/gather_res/res_taxa15/multires_multigroup/fixedeffectestimate_summary_fixed_realm.csv",sep=""),row.names = F)

#----------------Now, make table for Summary estimates from 100 runs--------------------------
fixedeffect<-read.csv(paste("../../Results/gather_res/res_taxa15/multires_multigroup/fixedeffectestimate_summary_fixed_realm.csv",sep=""))
...
sink(paste("../../Results/gather_res/res_taxa15/multires_multigroup/100runs_fixedeffects_estimates_table.txt",sep=""))
