...
reslocn<-paste("../../Results/gather_res/res_taxa15/multires_multigroup_traditional_stability")
...
call_multires_multigroup<-function(i){
  ...
  resloc<-paste("../../Results/gather_res/res_taxa15/multires_multigroup_traditional_stability/run_",i,sep="")
  ...
  bf_stability<-bf(stability ~ (R+A+LM)*REALM + (1|TAXA/UID))
  bf_LM<-bf(LM ~ R*REALM+(1|TAXA/UID))
...
for(i in c(1:100)){
  ...
  post<-post%>%dplyr::select(b_stability_Intercept,
                             b_stability_R,b_stability_A,b_stability_LM,
                             b_stability_REALMTerrestrial,
                             `b_stability_R:REALMTerrestrial`, `b_stability_A:REALMTerrestrial`, `b_stability_LM:REALMTerrestrial`,
                             b_LM_Intercept,b_A_Intercept,
                             b_LM_REALMTerrestrial,b_A_REALMTerrestrial,
                             b_LM_R,`b_LM_R:REALMTerrestrial`,
  ...
  rownames(df)<-c("Intercept_stability_Freshw",
                  "Slope_stability_R_Freshw","Slope_stability_A_Freshw","Slope_stability_LM_Freshw",
                  "Intercept_A_Freshw",
                  "Slope_A_R_Freshw",
                  "Intercept_LM_Freshw",
                  "Slope_LM_R_Freshw",
                  "Intercept_stability_Terres",
                  "Slope_stability_R_Terres","Slope_stability_A_Terres","Slope_stability_LM_Terres",
                  "Intercept_A_Terres",
                  "Slope_A_R_Terres",
                  "Intercept_LM_Terres",
                  "Slope_LM_R_Terres")
  
  df1<-post %>%dplyr::select(b_stability_Intercept)%>% 
    tidybayes::median_qi(.width = c(.95,0.75))
  df$Median[1]<-df1$b_stability_Intercept[1]
  df$LowCI0.95[1]<-df1$.lower[1]
  df$LowCI0.75[1]<-df1$.lower[2]
  df$UpCI0.95[1]<-df1$.upper[1]
  df$UpCI0.75[1]<-df1$.upper[2]
  
  df1<-post %>%dplyr::select(b_stability_R)%>% 
    tidybayes::median_qi(.width = c(.95,0.75))
  df$Median[2]<-df1$b_stability_R[1]
  df$LowCI0.95[2]<-df1$.lower[1]
  df$LowCI0.75[2]<-df1$.lower[2]
  df$UpCI0.95[2]<-df1$.upper[1]
  df$UpCI0.75[2]<-df1$.upper[2]
  
  df1<-post %>%dplyr::select(b_stability_A)%>% 
    tidybayes::median_qi(.width = c(.95,0.75))
  df$Median[3]<-df1$b_stability_A[1]
  df$LowCI0.95[3]<-df1$.lower[1]
  df$LowCI0.75[3]<-df1$.lower[2]
  df$UpCI0.95[3]<-df1$.upper[1]
  df$UpCI0.75[3]<-df1$.upper[2]
  
  df1<-post %>%dplyr::select(b_stability_LM)%>% 
    tidybayes::median_qi(.width = c(.95,0.75))
  df$Median[4]<-df1$b_stability_LM[1]
  ...
  df1<-post %>%dplyr::select(b_LM_Intercept)%>% 
    tidybayes::median_qi(.width = c(.95,0.75))
  df$Median[7]<-df1$b_LM_Intercept[1]
  df$LowCI0.95[7]<-df1$.lower[1]
  df$LowCI0.75[7]<-df1$.lower[2]
  df$UpCI0.95[7]<-df1$.upper[1]
  df$UpCI0.75[7]<-df1$.upper[2]
  
  df1<-post %>%dplyr::select(b_LM_R)%>% 
    tidybayes::median_qi(.width = c(.95,0.75))
  df$Median[8]<-df1$b_LM_R[1]
  df$LowCI0.95[8]<-df1$.lower[1]
  df$LowCI0.75[8]<-df1$.lower[2]
  df$UpCI0.95[8]<-df1$.upper[1]
  df$UpCI0.75[8]<-df1$.upper[2]
  
  df1<-post %>%mutate(b_stability_Intercept_terres=b_stability_Intercept+b_stability_REALMTerrestrial)%>% select(b_stability_Intercept_terres)%>% 
    tidybayes::median_qi(.width = c(.95,0.75))
  df$Median[9]<-df1$b_stability_Intercept_terres[1]
  df$LowCI0.95[9]<-df1$.lower[1]
  df$LowCI0.75[9]<-df1$.lower[2]
  df$UpCI0.95[9]<-df1$.upper[1]
  df$UpCI0.75[9]<-df1$.upper[2]
  
  df1<-post %>%mutate(b_stability_R_terres=b_stability_R+`b_stability_R:REALMTerrestrial`)%>% select(b_stability_R_terres)%>% 
    tidybayes::median_qi(.width = c(.95,0.75))
  df$Median[10]<-df1$b_stability_R_terres[1]
  df$LowCI0.95[10]<-df1$.lower[1]
  df$LowCI0.75[10]<-df1$.lower[2]
  df$UpCI0.95[10]<-df1$.upper[1]
  df$UpCI0.75[10]<-df1$.upper[2]
  
  df1<-post %>%mutate(b_stability_A_terres=b_stability_A+`b_stability_A:REALMTerrestrial`)%>% select(b_stability_A_terres)%>% 
    tidybayes::median_qi(.width = c(.95,0.75))
  df$Median[11]<-df1$b_stability_A_terres[1]
  df$LowCI0.95[11]<-df1$.lower[1]
  df$LowCI0.75[11]<-df1$.lower[2]
  df$UpCI0.95[11]<-df1$.upper[1]
  df$UpCI0.75[11]<-df1$.upper[2]
  
  df1<-post %>%mutate(b_stability_LM_terres=b_stability_LM+`b_stability_LM:REALMTerrestrial`)%>% select(b_stability_LM_terres)%>% 
    tidybayes::median_qi(.width = c(.95,0.75))
  df$Median[12]<-df1$b_stability_LM_terres[1]
  ...
  df1<-post %>%mutate(b_LM_Intercept_terres=b_LM_Intercept+b_LM_REALMTerrestrial)%>% select(b_LM_Intercept_terres)%>% 
    tidybayes::median_qi(.width = c(.95,0.75))
  df$Median[15]<-df1$b_LM_Intercept_terres[1]
  df$LowCI0.95[15]<-df1$.lower[1]
  df$LowCI0.75[15]<-df1$.lower[2]
  df$UpCI0.95[15]<-df1$.upper[1]
  df$UpCI0.75[15]<-df1$.upper[2]
  
  df1<-post %>%mutate(b_LM_R_terres=b_LM_R+`b_LM_R:REALMTerrestrial`)%>% select(b_LM_R_terres)%>% 
    tidybayes::median_qi(.width = c(.95,0.75))
  df$Median[16]<-df1$b_LM_R_terres[1]
  ...
  df<-df[c("Intercept_stability_Freshw","Intercept_stability_Terres",
           ...
           "Slope_stability_LM_Freshw","Slope_stability_LM_Terres",
           "Intercept_A_Freshw","Intercept_A_Terres",
           "Slope_A_R_Freshw","Slope_A_R_Terres",
           "Intercept_LM_Freshw","Intercept_LM_Terres",
           "Slope_LM_R_Freshw","Slope_LM_R_Terres"),]
...
write.csv(fixedeffect,paste("../../Results/gather_res/res_taxa15/multires_multigroup_traditional_stability/fixedeffectestimate_summary_fixed_realm.csv",sep=""),row.names = F)

#----------------Now, make table for Summary estimates from 100 runs--------------------------
fixedeffect<-read.csv(paste("../../Results/gather_res/res_taxa15/multires_multigroup_traditional_stability/fixedeffectestimate_summary_fixed_realm.csv",sep=""))
...
sink(paste("../../Results/gather_res/res_taxa15/multires_multigroup_traditional_stability/100runs_fixedeffects_estimates_table.txt",sep=""))
