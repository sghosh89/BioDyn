...
library(tidyverse)
#========================================================================================
sink("./console_brms_univar_model_stability_skw_ver2.txt", append=TRUE, split=TRUE)
#====================================================================================================
...
mydat<-sm_all[,c("siteid","iCV","iCValt","phi","REALM","phi_skw","nsp","L","U","f_nL","f_nU")]
...
model1<-lm(stability_skw ~ (R + VR)*REALM, mydat_scaled) # R:REALM + VR:REALM +REALM
model2<-lm(stability_skw ~ (R + VR + A)*REALM, mydat_scaled) 
model3<-lm(stability_skw ~ (R + VR + A + uniA)*REALM, mydat_scaled) 
model4<-lm(stability_skw ~ (R + VR + A + uniA + SR)*REALM, mydat_scaled) 
model5<-lm(stability_skw ~ (R + VR + SR)*REALM, mydat_scaled)
anova(model1,model2,model3,model4,model5)
AIC(model1,model2,model3,model4,model5)

cat(" ------ adjusted Rsq value for lm(stability_skw ~ (R + VR)*REALM, mydat_scaled)---------", "\n")
summary(model1)$adj.r.squared
cat(" ------ adjusted Rsq value for lm(stability_skw ~ (R + VR +A)*REALM, mydat_scaled)---------", "\n")
summary(model2)$adj.r.squared
cat(" ------ adjusted Rsq value for lm(stability_skw ~ (R + VR +A +uniA)*REALM, mydat_scaled)---------", "\n")
summary(model3)$adj.r.squared
cat(" ------ adjusted Rsq value for lm(stability_skw ~ (R + VR +A +uniA +SR)*REALM, mydat_scaled)---------", "\n")
summary(model4)$adj.r.squared
cat(" ------ adjusted Rsq value for lm(stability_skw ~ (R + VR +SR)*REALM, mydat_scaled)---------", "\n")
summary(model5)$adj.r.squared


#car::vif(lm(stability_skw ~ (R + VR), mydat_scaled)) testing multicolinearity 
#car::vif(lm(stability_skw ~ (R + VR+A+uniA+SR+REALM), mydat_scaled)) testing multicolinearity 
...
model_00<-brm(bf(stability_skw ~ (R + VR )),
              data=mydat_scaled,
...
model_01 <- update(model_00, stability_skw ~ (R + VR + A ), 
...
model_02 <- update(model_00, stability_skw ~ (R + VR + A + uniA), 
                   newdata = mydat_scaled)
...
model_03 <- update(model_00, stability_skw ~ (R + VR + A + uniA +SR), 
                   newdata = mydat_scaled)
...
model_04 <- update(model_00, stability_skw ~ (R + VR + SR), 
                   newdata = mydat_scaled)
...
model_00R<-update(model_00, stability_skw ~ (R + VR + REALM), 
                  newdata = mydat_scaled)
...
model_01R <- update(model_00, stability_skw ~ (R + VR + REALM + A ), 
                    newdata = mydat_scaled)
...
model_02R <- update(model_00, stability_skw ~ (R + VR+REALM + A + uniA), 
                    newdata = mydat_scaled)
...
model_03R <- update(model_00, stability_skw ~ (R + VR +REALM+ A + uniA +SR), 
                    newdata = mydat_scaled)
...
model_04R<-update(model_00, stability_skw ~ (R + VR +REALM + SR), 
                  newdata = mydat_scaled)
...
saveRDS(null_model,"../../Results/gather_res/null_model_skw_ver2.RDS")

cat(paste("========= starting a basic full_model Bayesian at ",Sys.time()," ============= \n"))

#bf_stability<-bf(stability_skw ~ (R + VR + SR + A + uniA )* REALM) 
#bform <- bf_stability 
...
full_model <- update(null_model, stability_skw ~ (R + VR + SR + A + uniA )* REALM, 
                     newdata = mydat_scaled)
...
saveRDS(full_model,"../../Results/gather_res/full_model_skw_ver2.RDS")
...
pdf("../../Results/gather_res/conditional_effects_brms_univar_skw_ver2.pdf",height=12,width=16)
