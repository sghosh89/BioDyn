...
library(tidyverse)
library(tidybayes)
resloc<-paste("../../Results/gather_res/res_model2668_fixed_realm_traditionalstability_without_BBS_data",sep="")
...
#--------- exclude BBS data ------------------
id<-startsWith(mydat$UID,"BBS")
idBBS<-which(id==T)
mydat<-mydat[-idBBS,]#1441 communities left

write.csv(mydat, paste(resloc,"/mydat_1441.csv",sep=""), row.names = F)
#====================================
mydat_scaled<-mydat
