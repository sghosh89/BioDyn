...
df<-df%>%filter(site==42)
# only one plot in this site, 30yrs, monthly freq = 1 
...
if(length(newsite)>1){
  for(k in 1:length(newsite)){
    resloc2<-paste("../../DATA/for_BioTIME/wrangled_data/Terrestrial_plotlevel/42/",newsite[k],"/",sep="")
...
saveRDS(newsite,"../../DATA/for_BioTIME/wrangled_data/Terrestrial_plotlevel/42/newsite.RDS")
