...
df<-df%>%filter(site==254)
# 2 sampling sites disconnected lakes

#df$newsite<-df$site # this is the same as there is single site
#----------- create result folder for wrangle ddata -------------------------
resloc<-"../../DATA/for_BioTIME/wrangled_data/Freshwater_plotlevel/254/"
...
for(k in 1:length(newsite)){
  resloc2<-paste("../../DATA/for_BioTIME/wrangled_data/Freshwater_plotlevel/254/",newsite[k],"/",sep="")
...
for(k in 1:length(newsite)){
  ...
  resloc<-paste("../../DATA/for_BioTIME/wrangled_data/Freshwater_plotlevel/254/",newsite[k],"/",sep="")
...
f<-paste("../../DATA/for_BioTIME/wrangled_data/Freshwater_plotlevel/samplingsite_254_selected.html",sep="")
