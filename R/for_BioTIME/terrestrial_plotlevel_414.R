...
df<-df%>%filter(site==414) # only one plot
...
if(length(newsite)>1){
  for(k in 1:length(newsite)){
    resloc2<-paste("../../DATA/for_BioTIME/wrangled_data/Terrestrial_plotlevel/414/",newsite[k],"/",sep="")
...
saveRDS(newsite,"../../DATA/for_BioTIME/wrangled_data/Terrestrial_plotlevel/414/newsite.RDS")
