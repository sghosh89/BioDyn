...
df<-df%>%filter(site==46)
# only one plot in this site, 47yrs, monthly freq = 1 

#----------- create result folder for wrangle ddata -------------------------
resloc<-"../../DATA/for_BioTIME/wrangled_data/Terrestrial_plotlevel/46/"
...
newsite<-site # as we know there is only one site

# Now, create folder for all these newsite
if(length(newsite)>1){
  for(k in 1:length(newsite)){
    resloc2<-paste("../../DATA/for_BioTIME/wrangled_data/Terrestrial_plotlevel/46/",newsite[k],"/",sep="")
...
for(k in 1:length(newsite)){
  ...
  if(length(newsite)>1){
    resloc<-paste("../../DATA/for_BioTIME/wrangled_data/Terrestrial_plotlevel/46/",newsite[k],"/",sep="")
  }else{
    resloc<-"../../DATA/for_BioTIME/wrangled_data/Terrestrial_plotlevel/46/"
  }
...
saveRDS(newsite,"../../DATA/for_BioTIME/wrangled_data/Terrestrial_plotlevel/46/newsite.RDS")
