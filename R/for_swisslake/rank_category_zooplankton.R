...
get_rankabun<-function(siteid){
  for(i in 1:length(siteid)){
    site<-siteid[i]
    if(i==2){ # for LU site
      i_mat<-readRDS(paste("../../DATA/for_swisslake/wrangled_data/zooplankton/input_mat_for_tail_analysis_",site,"_site3A01.RDS",sep=""))
    }else{
      i_mat<-readRDS(paste("../../DATA/for_swisslake/wrangled_data/zooplankton/input_mat_for_tail_analysis_",site,".RDS",sep=""))
    }
    ...
    saveRDS(rankabun,paste("../../Results/for_swisslake/zooplankton/",site,"/rankabun.RDS",sep=""))
...
siteid<-c("zoo_ZH","zoo_LU","zoo_SEM","zoo_HAL","zoo_GRE","zoo_BAL")
...
mypath<-"../../Results/for_swisslake/zooplankton/"
for(i in 1:length(siteid)){
...
# save the result
saveRDS(res_freshw,"../../Results/for_swisslake/interaction_freq_for_zooplankton.RDS")
