...
for(i in 1:length(good_TimeSeriesID_q3q4)){
  ...
  if(nyrused<yr_threshold){
    ...
    siteid_nbin4<-c(siteid_nbin4,siteid)
    resloc_output<-paste(resloc,siteid,"/","nbin4/",sep="")
    if(!dir.exists(resloc_output)){
      dir.create(resloc_output)
    }
    #----------- analysis with covary sp ----------------
    res<-tail_analysis(mat = df, resloc = resloc_output, nbin = 4)
    cat("---------- i= ",i," routeid=",siteid," ----------\n")
  }
}

saveRDS(siteid_nbin4,"../../Results/for_RivFishTIME/siteid_nbin4.RDS")
